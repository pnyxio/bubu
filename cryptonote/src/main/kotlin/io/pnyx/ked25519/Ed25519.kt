...
object FastHash: Hash {
    override fun invoke(vararg args: ByteArray): ByteArray {
        return ByteArray(32)
    }
}

fun ByteArray.reduceToScalar(): Scal {
    Assert.isTrue(this.size == 32)
    return Ed25519ScalarOps().reduce(this)
}
//a[0]+256*a[1]+...+256^31*a[31]
typealias UInt256 = ByteArray

fun UInt256.sc_check() : Boolean = true

typealias Scal = UInt256

fun random_scalar(): Scal = Ed25519.getRandomFieldElement().toByteArray()

internal fun Scal.toFe(): FieldElement = Ed25519.fieldEncoding.decode(this)

operator fun Scal.plus(el: Scal): Scal = this.toFe().add(el.toFe()).toByteArray()

operator fun Scal.minus(el: Scal): Scal = this.toFe().subtract(el.toFe()).toByteArray()

operator fun Scal.times(el: Scal): Scal = this.toFe().multiply(el.toFe()).toByteArray()

operator fun Scal.times(point: GePoint): ScalPointMul {
    return ScalPointMul(this, point)
}

fun Scal.isnonzero(): Boolean = this.toFe().isNonZero()

typealias CompressedPoint = ByteArray

interface Point {
    fun toP2(): P2
    fun toCompressedPoint(): CompressedPoint
}


class Signature(val c: Scal, val r: Scal)

abstract class GePoint(internal val ge: GroupElement): Point {

    override fun toCompressedPoint(): CompressedPoint = ge.toByteArray()

}

class ScalPointMul(private val scal: Scal, private val point: GePoint) : Point {

    private val p3: P3 get() = P3(point.ge.scalarMultiply(scal))

    override fun toP2(): P2 = p3.toP2()
    fun toP3(): P3 = p3

    override fun toCompressedPoint(): CompressedPoint = p3.toCompressedPoint()
    ...
    operator fun plus(pt: Point): GePoint = when {
        ...
        pt is ScalPointMul -> {
            //TODO check this  and A precomputed
            P2(point.ge.doubleScalarMultiplyVariableTime(pt.point.ge, pt.scal, scal))
        }
...
class P3 internal constructor(ge: GroupElement) : GePoint(ge) {
    init {
        Assert.isTrue(ge.representation == GroupElement.Representation.P3)
    }
    constructor(pt: CompressedPoint) : this(GroupElement(Ed25519.curve, pt))
    constructor(pt: CompressedPoint, precomp: Boolean) : this(GroupElement(Ed25519.curve, pt, precomp))
...
class P2 internal constructor(ge: GroupElement) : GePoint(ge) {
    init {
        Assert.isTrue(ge.representation == GroupElement.Representation.P2)
    }
    override fun toP2(): P2 = P2(ge.toP2())
    fun mul8(): P1P1 = dbl().toP2().dbl().toP2().dbl()
...
class P1P1 internal constructor(ge: GroupElement) : GePoint(ge) {
    init {
        Assert.isTrue(ge.representation == GroupElement.Representation.P1P1)
    }
...
class GeCached internal constructor(ge: GroupElement) : GePoint(ge) {
    init {
        Assert.isTrue(ge.representation == GroupElement.Representation.CACHED)
    }
...
typealias SecretKey = Scal


typealias KeyDerivation = ByteArray

fun KeyDerivation.toScalar(output_index: Int): Scal {
    //TODO
//        struct {
...
object Ed25519 {
    ...
    val curve = edDSANamedCurveSpec.curve
    val fieldEncoding = edDSANamedCurveSpec.curve.field.encoding
    //val B = edDSANamedCurveSpec.b//base point
    val B = P3(edDSANamedCurveSpec.b)
...
// region group element
    ...
    fun toGroupElement(bytes: ByteArray): GroupElement {
        ...
        val y = Ed25519.toBigInteger(bytes)
